...
class IslandDao(BaseModel):
    ...
    async def get(self, world_id: str, island_id: str) -> WrappedData[IslandLite]:
    ...
    async def post(self, world_id: str, island: IslandLite) -> None:
        ...
        stored_island: WrappedData[IslandLite] = await self.get(world_id=world_id, island_id=island.id)
        if stored_island.nonce != nonce:
            msg: str = f"storage inconsistency detected while storing island {island.id} - nonce mismatch!"
            raise DaoInconsistencyError(msg)

    async def put_safe(self, world_id: str, wrapped_island: WrappedData[IslandLite]) -> None:
        ...
        # now validate we stored
        stored_island: WrappedData[IslandLite] = await self.get(world_id=world_id, island_id=wrapped_data.data.id)
        if stored_island.nonce != nonce:
            msg: str = (
