...
SESSION_ID=$(screen -ls | grep mongodb-local | awk '{print $1}')
echo
if [ -n "$SESSION_ID" ]; then
  ...
  screen -dmS mongodb-local -h 1000 infra/local/mongodb-$MONGODB_LOCAL/bin/mongod --dbpath infra/local/data
fi
